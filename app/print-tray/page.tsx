...
export default function PrintTrayPage() {
  ...
  const { finalImageUrl, photos, selectedLayout, selectedFrame, selectedFilter, resetSession } = usePhotoboothStore();
  ...
  const renderPhoto = (index: number, style: React.CSSProperties) => (
    <div key={index} style={{ ...style, backgroundColor: "#1C1B1A", overflow: "hidden" }}>
      {photos[index] ? (
        <img src={photos[index]} alt={`Photo ${index + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", filter: FILTER_CSS[selectedFilter] }} />
      ) : (
  ...
  return (
    <main style={{ minHeight: "100vh", backgroundColor: "#F4F1EA", display: "flex", flexDirection: "column", alignItems: "center", position: "relative", overflow: "auto", paddingTop: "80px", paddingBottom: "80px" }}>
